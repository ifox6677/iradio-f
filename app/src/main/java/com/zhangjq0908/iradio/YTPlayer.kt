...
object YTPlayer {
    ...
    suspend fun ytPlay(
        ...
        try {
            ...
            withContext(Dispatchers.Main) {
                Toast.makeText(context, "播放已取消", Toast.LENGTH_SHORT).show()
            }
        } catch (e: Exception) {
    ...
    private fun parseYouTube(ytUrl: String): Triple<String, String, String> {
        ...
        // 调试日志：显示排序结果
        //Log.d(TAG, "=== 音频流选择调试 ===")
        //Log.d(TAG, "找到 ${validAudioStreams.size} 个有效流")
        sortedStreams.take(3).forEachIndexed { index, stream ->
            ...
            //Log.d(TAG, "TOP${index + 1}: 格式=$ext, 比特率=${abr}kbps, 编码=$acodec")
        }
        ...
        //Log.d(TAG, "最终选择: 格式=$selectedExt, 比特率=${selectedAbr}kbps")
    ...
    private suspend fun playStream(
        ...
        val isLifecycleValid = lifecycleOwner?.lifecycle?.currentState?.isAtLeast(Lifecycle.State.STARTED)
            ?: true
        if (!isLifecycleValid) {
            Toast.makeText(context, "页面已关闭，无法播放", Toast.LENGTH_SHORT).show()
            return@withContext
        }
