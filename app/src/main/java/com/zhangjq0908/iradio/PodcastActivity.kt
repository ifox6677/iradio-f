...
import com.zhangjq0908.iradio.download.PodcastDownloadManager
import com.zhangjq0908.iradio.download.PodcastFileHelper
import com.zhangjq0908.iradio.download.DownloadState
import com.zhangjq0908.iradio.download.DownloadBroadcast
import java.io.File
...
class PodcastActivity : AppCompatActivity() {
        ...
        override fun onReceive(context: Context?, intent: Intent?) {
            when (intent?.action) {
                ...
                "SEEK_COMPLETED" -> {
                    ...
                    //Log.d("PodcastActivity", "跳转完成到: ${progressBarManager.formatTime(seekPosition)}")
                }
            }
        }
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.playModeButton.setOnClickListener {
            ...
            //showQuickToast(if (isContinuousMode) "已启用连续播放" else "已启用单集播放")
        }
        ...
        lifecycleScope.launch {
            ...
            if (parseResult == null) {
                //showErrorToast("加载失败，请检查网络或 RSS 地址")
                return@launch
            }
    ...
    private fun bindFeed(parseResult: RssParser.ParseResult) {
        ...
        if (episodes.isEmpty()) {
            //showErrorToast("RSS 暂无内容")
        } else {
            //showQuickToast("已加载 ${episodes.size} 个剧集")
        }
    ...
    private fun performSeek(position: Long) {
        ...
        //showQuickToast("跳转到: ${progressBarManager.formatTime(position)}", Toast.LENGTH_SHORT)
    }
                ...
                when (episode.downloadState) {
                    DownloadState.NONE, DownloadState.FAILED -> {
                        ...
                        if (lower.endsWith(".m4a") || lower.endsWith(".mp3") || lower.endsWith(".aac") || lower.endsWith(".ogg") || lower.contains("xyzcdn.net")) {
                            ...
                            //showQuickToast("开始下载: ${episode.title}")
                        } else if (url.contains("youtube.com") || url.contains("youtu.be")) {
                            //showQuickToast("解析 YouTube 链接中...", Toast.LENGTH_SHORT)
                            this@PodcastActivity.lifecycleScope.launch {
                                try {
                                    ...
                                    if (!realUrl.isNullOrBlank()) {
                                        ...
                                        notifyItemChanged(adapterPosition)
                                        //showQuickToast("解析失败，无法下载")
                                    }
                                } catch (e: Exception) {
                                    episode.downloadState = DownloadState.FAILED
                                    notifyItemChanged(adapterPosition)
                                    //showQuickToast("下载失败: ${e.message}")
                                }
                            }
                        } else {
                            PodcastDownloadManager.download(holder.itemView.context, episode.title, url)
                    ...
                    DownloadState.DOWNLOADING -> {}//
                    DownloadState.COMPLETED -> {}//showQuickToast("已下载完成", Toast.LENGTH_SHORT)
                }
            }
