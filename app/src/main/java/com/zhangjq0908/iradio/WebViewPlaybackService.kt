...
class WebViewPlaybackService : Service() {
    ...
    override fun onCreate() {
        ...
        //Log.d(TAG, "WebViewPlaybackService onCreate")
    }
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        return when (intent?.action) {
            ...
            ACTION_PLAY_STREAM -> {
                ...
                //Log.d(TAG, "收到播放请求: $title, URL: ${url?.take(90)}..., source: $source") // ------
    ...
    private fun updatePlayerState(state: Int) {
        ...
        //Log.d(TAG, "播放状态变化: ${getStateName(oldState)} -> ${getStateName(state)}, isStreamPlaying=$isStreamPlaying")
            ...
            player = ExoPlayer.Builder(this).build().apply {
                ...
                addListener(object : Player.Listener {
                    override fun onPlaybackStateChanged(playbackState: Int) {
                        //Log.d(TAG, "ExoPlayer state: $playbackState (${getStateName(playbackState)})")
...
/*    private fun showQuickToast(message: String) {
        coroutineScope.launch {
            try {
                Toast.makeText(this@WebViewPlaybackService, message, Toast.LENGTH_SHORT).show()
            } catch (e: Exception) {
                Log.e(TAG, "显示提示失败", e)
            }
        }
...
*/
    ...
    override fun onDestroy() {
        //Log.d(TAG, "WebViewPlaybackService onDestroy")
